# code is copied from https://github.com/Trusted-AI/adversarial-robustness-toolbox,
# and some modifications have been made to enable it to handle non-image data.
"""
    ...
    def generate(self, x: np.ndarray, y: Optional[np.ndarray] = None, **kwargs) -> np.ndarray:
        ...
        for _ in trange(max(1, self.nb_random_init), desc="AutoPGD - restart", disable=not self.verbose):
            ...
            for batch_id in trange(
                ...
                tmp = [1 for i in range(len(x_k.shape) - 1)]
                eta = np.full(tuple([_batch_size] + tmp), self.eps_step).astype(ART_NUMPY_DTYPE)
